...
class DetectorObstaculos(object):
	...
	def process(self, data):
		...
		self.obstacle = ( np.greater( sub_image, 0.2 ) * np.less( sub_image, 0.52 ) ).any()
		if self.obstacle:
	...
	def calculate_route(self):
		...
		new_route = [ [new_vectors[0][0][0],new_vectors[0][1][0],None] ,[new_vectors[1][0][0],new_vectors[1][1][0] ,None]]
		print(new_route)
